...
class main
{
    ...
    public function handle($mode = "")
    {
        ...
        if (sizeof($forum_array)) {
            $sql_and =
                " AND " . $db->sql_in_set("p.forum_id", $forum_array, true);
        }
        ...
        if ($mode == "topx_newpost") {
            $tiep = false;
            $sql_select =
                "MAX(p.post_time) as post_time, u.user_id, u.username, u.user_colour, t.topic_title, t.forum_id, t.topic_last_post_id, t.topic_views, t.topic_replies, f.forum_name, t.topic_last_poster_id, t.topic_last_poster_name, t.topic_last_poster_colour,t.topic_time";
            $sql_order = "post_time DESC";
        } elseif ($mode == "topx_mostview") {
            $sql_select =
                "MAX(t.topic_views) as topic_views, u.user_id, u.username, u.user_colour, t.topic_title, t.forum_id, t.topic_last_post_id, p.post_time, t.topic_replies, f.forum_name, t.topic_last_poster_id, t.topic_last_poster_name, t.topic_last_poster_colour,t.topic_time";
            $sql_order = "topic_views DESC";
            $tiep = false;
        } elseif ($mode == "topx_hotesttopic") {
            $sql_select =
                "MAX(t.topic_replies) as topic_replies, u.user_id, u.username, u.user_colour, t.topic_title, t.forum_id, t.topic_last_post_id, t.topic_views, p.post_time, f.forum_name, t.topic_last_poster_id, t.topic_last_poster_name, t.topic_last_poster_colour,t.topic_time";
        ...
        if (!$tiep) {
            ...
            while ($row = $db->sql_fetchrow($result)) {
                $shortusername =
                    utf8_strlen($row["topic_last_poster_name"]) >
                    $char_username_limit
                        ? utf8_substr(
                                $row["topic_last_poster_name"],
                                0,
                                $char_username_limit - 3
                            ) . "..."
                        : $row["topic_last_poster_name"];
                ...
                $latest_poster = get_username_string(
                    "full",
                    $row["topic_last_poster_id"],
                    $row["topic_last_poster_name"],
                    $row["topic_last_poster_colour"]
                );
                $this->template->assign_block_vars("topic_rows", [
                    ...
                    "TOPIC_TITLE" =>
                        utf8_strlen($topic_title) > $char_limit
                            ? utf8_substr($topic_title, 0, $char_limit - 3) .
                                "..."
                            : $topic_title,
            ...
            switch ($mode) {
                case "newest":
                    if (
                        ($newest_users = $cache->get("_topx_newest_users")) ===
                        false
                    ) {
                        $newest_users = [];
                        ...
                        $sql =
                            ...
                            '
                                                WHERE ' .
                            $db->sql_in_set("user_type", $ignore_users, true) .
                            '
                    ...
                    foreach ($newest_users as $row) {
                        $shortusername =
                            utf8_strlen($row["username"]) > $char_username_limit
                                ? utf8_substr(
                                        $row["username"],
                                        0,
                                        $char_username_limit - 3
                        ...
                        $this->template->assign_block_vars("topx_newest", [
                ...
                case "topposter":
                    if (
                        ($user_posts = $cache->get("_topx_posters")) === false
                    ) {
                        $user_posts = [];
                    ...
                    foreach ($user_posts as $row) {
                        ...
                        $this->template->assign_block_vars("topx_active", [
                            "S_SEARCH_ACTION" => append_sid(
                                "{$phpbb_root_path}search.$phpEx",
                                "author_id=" . $row["user_id"] . "&amp;sr=posts"
                            ),
                ...
                case "topthanked":
                    if (
                        ($user_thanked = $cache->get("_topx_thanked")) === false
                    ) {
                        $user_thanked = [];

                        $sql =
                            'SELECT user_id, username, user_colour, user_thanked
                                                FROM ' .
                            USERS_TABLE .
                            '
                                                WHERE ' .
                    ...
                    foreach ($user_thanked as $row) {
                        ...
                        $this->template->assign_block_vars("topx_thanked", [
                            "S_SEARCH_ACTION" => append_sid(
                                "{$phpbb_root_path}search.$phpEx",
                                "author_id=" . $row["user_id"] . "&amp;sr=posts"
                            ),
